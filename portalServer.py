    ...
    def do_POST(self):
        ...
        try:
            if self.path == '/addAccount':
                ...
                '''
                self.database.addAccount(ownerName, owner_ssn, balance, acct_status)
        ...
        if self.path == '/searchTransactions':
                    ...
                    '''
                        Example call: self.database.addAccount(student_name, ownerName, owner_ssn, balance, status)
                    '''
                    self.database.accountTransactions(self, accountID)
                    ...
                    self.wfile.write(b"<div> <a href='/'>Home</a>| \
                                     <a href='/addAccount'>Add Account</a>|\
                                      ...
                                      <a href='/deletAccount'>Delete Account</a></div>")
                    self.wfile.write(b"<hr>")
                    self.wfile.write(b"<h3>Account Transactions</h3>")
                    self.wfile.write(b"<div><a href='/searchTransactions'>Look up another account?</a></div>")
                    self.wfile.write(b"</center></body></html>")
    ...
    def do_GET(self):
        try:
            ...
            if self.path == '/withdraw':
                ...
                self.wfile.write(b'<label for="accountID">Account ID Number:</label>\
                                  <input type="number" id="accountID" name="accountID"><br><br>\
                                  <label for="withdraw">Withdraw amount:</label>\
                                  <input type="number" id="withdraw" name="withdraw"><br><br>\
                                  <input type="submit" value="Submit">\
                                  </form>')
            ...
            if self.path =='/deletAccount':
                ...
                self.wfile.write(b"<div> <a href='/'>Home</a>| \
                                 <a href='/addAccount'>Add Account</a>|\
                                  ...
                                  <a href='/searchTransactions'>Search Transactions</a>|\
                                  <a href='/deletAccount'>Delete Account</a></div>")
                self.wfile.write(b"<hr><h2>Delete Your Account</h2>")
                self.wfile.write(b"<form action='/deletAccount' method='post'>")
                self.wfile.write(b'<label for="accountID">Account ID Number:</label>\
                                  <input type="number" id="accountID" name="accountID"><br><br>\
